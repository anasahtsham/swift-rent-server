...
//       const { email: userStoredEmail, phone: userStoredPhone } =
//         userCredentialsQuery.rows[0];

//       // Check if both email and phone are unchanged
//       if (
//         (email === userStoredEmail ||
//           (email === "" && userStoredEmail === null)) &&
//         (phone === userStoredPhone ||
//           (phone === "" && userStoredPhone === null))
//       ) {
//         return res.status(200).json({ success: true });
//       } else {
...
//     return res
//       .status(200)
//       .json({ userID, ownerID, tenantID, digiCode, success: true });
//   } catch (error) {
...
//   const firstName =
//     spaceIndex === -1 ? userName : userName.substring(0, spaceIndex);
...
//     res
//       .status(200)
//       .json({ ownerName: ownername, email, phone, DOB, success: true });
//   } catch (error) {
...
//     res
//       .status(200)
//       .json({ tenantName: tenantname, email, phone, DOB, success: true });
//   } catch (error) {
...
//     await db.query(
//       "UPDATE UserInformation SET md5password = $1 WHERE id = $2",
//       [newPassword, userQuery.rows[0].id]
//     );
...
//       return res
//         .status(404)
//         .json({ error: "User is not registered as a tenant" });
//     }
...
//       if (
//         ownerUserQuery.rows.length > 0 &&
//         ownerUserQuery.rows[0].userid === userID
//       ) {
//         return res
//           .status(401)
//           .json({
//             error: "Owner cannot register as a tenant to their own property",
//           });
//       }
//     }

//     // Update the property's tenantID with the found tenantID
//     await db.query("UPDATE Property SET tenantID = $1 WHERE id = $2", [
//       tenantID,
//       propertyID,
//     ]);
...
//     await db.query("UPDATE Property SET tenantID = 0 WHERE id = $1", [
//       propertyID,
//     ]);
...
//     return res
//       .status(200)
//       .json({ bugData: complaintsQuery.rows, success: true });
//   } catch (error) {
...
//     return res
//       .status(200)
//       .json({ totalUsers, totalOwners, totalTenants, success: true });
//   } catch (error) {
...
//     await db.query(
//       "UPDATE UserInformation SET md5password='00000000000000000000000000000000' WHERE id = $1",
//       [userID]
//     );
...
//     const userQuery = await db.query(
//       "SELECT email FROM UserInformation WHERE id = $1",
//       [userID]
//     );
...
//             AND amount != 1;
//         `;
//         const monthlyProfitResult = await db.query(monthlyProfitQuery, [
//           year,
//           month,
//         ]);
...
//     const bugQuery = await db.query("SELECT * FROM ReportedBug WHERE id = $1", [
//       bugID,
//     ]);
...
//     await db.query("UPDATE ReportedBug SET bugStatus=$1 WHERE id = $2", [
//       bugStatus,
//       bugID,
//     ]);
...
// app.post("/api/tenant/rental-list", async (req, res) => {
//   // Input
//   const { tenantID } = req.body;
...
//       return res
//         .status(400)
//         .json({ error: "No properties found for the tenant" });
//     }
//   } catch (error) {
...
//       return res
//         .status(400)
//         .json({ error: "Unable to create rent transaction" });
//     }
//   } catch (error) {
...
//     await db.query("UPDATE Property SET dueDate = '0' WHERE id = $1", [
//       propertyID,
//     ]);
